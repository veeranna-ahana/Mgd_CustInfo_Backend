...
taxdbRouter.post('/alltaxdetails', async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod("SELECT TaxName,Tax_Percent FROM magodmis.taxdb where current_date() >= EffectiveFrom and current_date() <= EffectiveTO order by TaxName asc", (err, data) => {
            if (err) console.log(err)
            res.send(data);
         //   console.log(data);
        })
